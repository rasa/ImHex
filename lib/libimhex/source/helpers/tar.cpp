...
namespace hex {
    ...
    Tar::Tar(const std::fs::path &path, Mode mode) {
        ...
        this->m_ctx = std::make_unique<mtar_t>();

        auto shortPath = wolv::io::fs::toShortPath(path);
        if (mode == Tar::Mode::Read)
            tar_error = mtar_open(this->m_ctx.get(), shortPath.string().c_str(), "r");
        else if (mode == Tar::Mode::Write)
            tar_error = mtar_open(this->m_ctx.get(), shortPath.string().c_str(), "a");
        else if (mode == Tar::Mode::Create)
            tar_error = mtar_open(this->m_ctx.get(), shortPath.string().c_str(), "w");
        else
    ...
    Tar::Tar(hex::Tar &&other) noexcept {
        this->m_ctx = std::move(other.m_ctx);
